...
export default function Dock() {
  return (
    <nav aria-label="Quick navigation" className="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 lg:bottom-6">
      {/* Mobile: icon + label, core links plus a direct WhatsApp action */}
      <ul className="flex items-center gap-0.5 rounded-full border border-border/60 bg-background/90 p-2 shadow-warm backdrop-blur-lg lg:hidden">
        {dockLinks
          .filter((l) => l.core)
          .map(({ to, label, icon: Icon }) => (
            <li key={to}>
              <Link
                ...
                className="flex h-14 w-16 flex-col items-center justify-center gap-1 rounded-full text-foreground/60 transition-colors hover:text-accent"
        ...
        <li>
          <a
            ...
            className="flex h-14 w-16 flex-col items-center justify-center gap-1 rounded-full bg-green-500 text-white transition-transform hover:scale-105"
          >
